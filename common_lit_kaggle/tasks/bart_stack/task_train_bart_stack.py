...
class TrainBartStackTask(Task):
    ...
    def run(self, context: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        bart_model_left: BartModel = BartModel.from_pretrained(
            model_path, config=bart_config
        )
        bart_model_right: BartModel = BartModel.from_pretrained(
            model_path, config=bart_config
        )
        bart_model_left.to(config.device)
        bart_model_right.to(config.device)
        bart_model_left.train()
        bart_model_right.train()

        bart_stack = BartStackWithRegressionHead(
            config=bart_config, barts=[bart_model_left, bart_model_right]
        )
