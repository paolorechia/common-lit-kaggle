...
def dataframe_to_tensors(
    ...
    input_ids_list = []
    for text in input_data.select(pl.col(source_column)).to_numpy():
...
class PrepareTensorTrainTwinsDataTask(Task):
    def __init__(
        self,
        name: str | None = None,
        truncation_length: int = 0,
        unified_text_data_key="train_data",
        output_text_data_key="tensor_train_data",
    ) -> None:
        super().__init__(name)
        self.truncation_length: Optional[int] = truncation_length
        self.unified_text_data_key = unified_text_data_key
        self.output_text_data_key = output_text_data_key

        assert self.truncation_length > 0, "Set string length truncation first!"
    ...
    def run(self, context: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        tensor_train_data = TensorDataset(
            prompt_ids_stack.to(config.device),
            answer_ids_stack.to(config.device),
            stacked_labels.to(config.device),
        )
        return {
            "tokenizer": bart_tokenizer,
            self.output_text_data_key: tensor_train_data,
        }
