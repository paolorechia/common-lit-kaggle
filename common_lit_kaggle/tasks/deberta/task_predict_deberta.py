...
from common_lit_kaggle.modeling import DebertaWithRegressionHead
from common_lit_kaggle.settings.config import Config

logger = logging.getLogger(__name__)


class PredictDebertaTask(Task):
    def __init__(
    ...
    def run(self, context: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        model = DebertaWithRegressionHead.from_pretrained(model_path)
        ...
        for tensor in tqdm(tensors_to_predict):
            ...
            result = model.forward(tensor.reshape(1, config.model_context_length))
