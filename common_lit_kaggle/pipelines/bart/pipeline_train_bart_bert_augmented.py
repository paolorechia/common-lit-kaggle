...
class TrainBartWithBertAugmentationPipeline(Pipeline):
    def __init__(self) -> None:
        config = Config.get()
        mlflow.set_tags({"name": config.model, "preprocessing": "bert_augmentation"})
        mlflow.log_params(
            {
        ...
        super().__init__(
            ...
            [
                ...
                data_split.ReadBertTrainTask(),
                data_split.MergeAugmentedSourcesTask(
                    data_sources=["bert_augmented_train_data"]
                ),
